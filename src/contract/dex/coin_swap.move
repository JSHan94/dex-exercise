module addr::CoinSwap {
    ...
    struct Capabilities<phantom CoinLP> has key {
        ...
        freeze_cap: FreezeCapability<CoinLP>,
    ...
    struct FakeCoinACapabilities has key {
        ...
        freeze_cap: FreezeCapability<FakeCoinA>,
    ...
    struct FakeCoinBCapabilities has key {
        ...
        freeze_cap: FreezeCapability<FakeCoinB>,
    ...
    public fun create_fake_coins(
        ...
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<FakeCoinA>(
            source,
            name,
            symbol,
            6,
        ...
        coin::register<FakeCoinA>(source);
        ...
        let (burn_cap, freeze_cap, mint_cap) = coin::initialize<FakeCoinB>(
            source,
            name,
            symbol,
            6,
        ...
        coin::register<FakeCoinB>(source);
    ...
    #[test(account = @0x2)]
    #[expected_failure(abort_code = ENO_ALREADY_INITIALIZED)]
    ...
    #[test(account = @0x2)]
    #[expected_failure(abort_code = ENO_MINIMUM_RECEIVE)]
    ...
    public entry fun test_swap_reverse(
        ...
        let (amount_a, amount_b, amount_lp) = pool_info<FakeCoinA, FakeCoinB, FakeCoinLP>(account_address);
        assert!(amount_b == 110 * 1000000u64, 4);
        assert!(amount_a == 90909090u64, 5);
        assert!(amount_lp == 100 * 1000000u128, 6);
    }
